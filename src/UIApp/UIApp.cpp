...
void BusySpin(UINT milliseconds)
{
    ...
    const long long target = start.QuadPart + static_cast<long long>(milliseconds) * freq.QuadPart / 1000;
...
void CreateControls(HWND hwnd)
{
    ...
    g_editInterval = CreateWindowExW(
        WS_EX_CLIENTEDGE, L"EDIT", L"3000",
        WS_CHILD | WS_VISIBLE | WS_TABSTOP | ES_AUTOHSCROLL | ES_RIGHT | ES_NUMBER,
        0, 0, 0, 0,
        hwnd, reinterpret_cast<HMENU>(static_cast<INT_PTR>(ID_EDIT_INTERVAL)),
        instance, nullptr);
    ...
    g_btnSleep = CreateWindowExW(
        ...
        0, 0, 0, 0,
        hwnd, reinterpret_cast<HMENU>(static_cast<INT_PTR>(ID_BTN_SLEEP)),
        instance, nullptr);
    ...
    g_btnWait = CreateWindowExW(
        ...
        WS_CHILD | WS_VISIBLE | WS_TABSTOP | BS_AUTOCHECKBOX | BS_PUSHLIKE,
        0, 0, 0, 0,
        hwnd, reinterpret_cast<HMENU>(static_cast<INT_PTR>(ID_BTN_WAIT)),
        instance, nullptr);

    g_btnCpu = CreateWindowExW(
        0, L"BUTTON", L"CPU hog",
        WS_CHILD | WS_VISIBLE | WS_TABSTOP | BS_AUTOCHECKBOX | BS_PUSHLIKE,
        0, 0, 0, 0,
        hwnd, reinterpret_cast<HMENU>(static_cast<INT_PTR>(ID_BTN_CPU)),
        instance, nullptr);
...
void LayoutControls(HWND hwnd)
{
    ...
    int x = margin;
    MoveWindow(g_editInterval, x, bottomY, (editWidth > 0 ? editWidth : 0), rowHeight, TRUE);
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_PAINT:
    {
        ...
        wchar_t buffer[64];
        wsprintfW(buffer, L"%02d:%02d:%02d.%01d",
                  st.wHour, st.wMinute, st.wSecond, st.wMilliseconds / 100);

        HFONT font = g_clockFont ? g_clockFont : reinterpret_cast<HFONT>(GetStockObject(DEFAULT_GUI_FONT));
                      ...
                      _In_ int nCmdShow)
{
    ...
    HWND hwnd = CreateWindowExW(
        ...
        WS_OVERLAPPEDWINDOW,
        CW_USEDEFAULT, CW_USEDEFAULT, 580, 240,
