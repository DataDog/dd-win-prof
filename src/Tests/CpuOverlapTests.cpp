...
TEST(CpuOverlapTests, OverlapDetection_LosesPrecisionOnSubMillisecondDifferences) {
  ...
  auto result = ComputeCpuForSampleWithOverlapDetection(
...
TEST(CpuOverlapTests, OverlapDetection_DocumentsPrecisionLossIssue) {
  ...
  TestCase cases[] = {
      {1'000'000ns, 1'100'000ns, 1ms, 0,
       "100µs difference → 0ms (PRECISION LOSS)"},
      {1'000'000ns, 1'500'000ns, 1ms, 0,
       "500µs difference → 0ms (PRECISION LOSS)"},
      {1'000'000ns, 1'999'000ns, 1ms, 0,
       "999µs difference → 0ms (PRECISION LOSS)"},
      {1'000'000ns, 2'000'000ns, 1ms, 0,
       "1000µs difference → 0ms (EDGE CASE)"},
      {1'000'000ns, 2'001'000ns, 1ms, 1, "1001µs difference → 1ms (OK)"},
  };

  for (const auto& tc : cases) {
    auto result = ComputeCpuForSampleWithOverlapDetection(
        tc.initialCpu, tc.lastTs, tc.thisTs
    );
...
TEST(CpuOverlapTests, FixedVersion_PreservesPrecision) {
  ...
  EXPECT_EQ(result.count(), 499'000)
      << "Fixed version preserves nanosecond precision";
